...
class EnhancedFlowerClient(fl.client.NumPyClient):
    """Enhanced Flower client supporting FedAvg, FedProx, and AsyncFL algorithms"""
    ...
    def __init__(self, model, train_loader, test_loader, device, client_id):
        ...
        logger.info(f"Client {client_id} zero-day scenario: missing {self.missing_attack}")
...
def main():
    ...
    try:
        ...
        # Validate data size
        if len(X) < 50:
            logger.warning(f"Client {CLIENT_ID} has very few samples ({len(X)}). This may affect performance.")
